...
export async function onRequestPost(context) {
  ...
  // subrequest budget. Same pattern as meta-instagram.js.
  ...
  const ws = await env.DB.prepare(`
    SELECT workspace_id, meta_page_id
      FROM workspace_config
     WHERE workspace_id = ? AND meta_page_id IS NOT NULL AND meta_page_id != ''
  `).bind(body.workspace_id).first();
  if (!ws) return json({ ok: true, skipped: true, reason: 'workspace not found or has no meta_page_id' });
